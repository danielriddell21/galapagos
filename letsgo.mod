...
// CGO_ENABLED=0 — but widening the list is a decision about what this project
...
// `galapagos`, so the command does not depend on which one you installed.
variant gui (
...
// The browser demo is compiled on demand rather than shipped, so the formula
// says where it actually lives. Needs letsgo v0.8.0 or later.
brew caveats "`galapagos serve` compiles the browser demo on demand, so it needs Go and a checkout of the repository."
